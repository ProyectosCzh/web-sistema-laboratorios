...
const emptyForm: TeacherFormState = { code: "", name: "", email: "", active: true };

export default function TeachersPanel() {
  const queryClient = useQueryClient();

  const [modal, setModal] = useState<{ mode: "create" } | { mode: "edit"; teacher: Teacher } | null>(null);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (modal?.mode === "edit") {
        await api.patch(`/teachers/${modal.teacher.id}`, body);
      } else {
  ...
  if (isLoading) {
    return <p className="text-sm text-gray-500">Cargando docentes...</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {teachers.length === 0 ? (
        <p className="text-sm text-gray-500">Sin docentes cargados.</p>
      ) : (
            ...
            <tbody>
              {teachers.map((teacher) => (
                <tr key={teacher.id} className="border-b border-gray-100">
                  <td className="px-4 py-2 font-medium text-gray-900">{teacher.code}</td>
                  <td className="px-4 py-2 text-gray-700">{teacher.name}</td>
                  <td className="px-4 py-2 text-gray-700">{teacher.email ?? "—"}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-medium ${
                        teacher.active ? "bg-emerald-100 text-emerald-800" : "bg-gray-200 text-gray-700"
                      }`}
                    >
      ...
      {modal && (
        <Modal title={modal.mode === "edit" ? "Editar docente" : "Nuevo docente"} onClose={() => setModal(null)}>
